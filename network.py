...
class PyramidNet(nn.Module):
    ...
    def __init__(self, n_layers, input_image_channels=3, output_channels=1, loss_weights=None):  # n_layers=5 5层网络
        ...
        self.no_channels = 32 # todo:测试通道数影响???? 结论：没啥影响
    ...
    def compute_multiscale_loss_experiment(self, multiscale_prediction, multiscale_targets, multiscale_masks):
        ...
        for i,(x,y,mask) in enumerate(zip(multiscale_prediction, multiscale_targets, multiscale_masks)):
            ...
            # # label = label.long()
            # y_wight = y.float()
            # num_positive = torch.sum((y_wight == 1).float()).float()
            # num_negative = torch.sum((y_wight == 0).float()).float()
            #
            # y_wight[y_wight == 1] = 1.0 * num_negative / (num_positive + num_negative)  # todo：mask是干啥的？不理解这种写法
            # y_wight[y_wight == 0] = 1.1 * num_positive / (num_positive + num_negative)
            # y_wight[y_wight == 2] = 0
            # cost = torch.nn.functional.binary_cross_entropy(  # mask是精确到像素点级别的权重
            #     x* mask, y* mask, weight=y_wight, reduce=False)
            ...
            # num_not_boundary_point=0.0
            # num_pre_point=0.0
            # for ii in range(1, x_mask.shape[0]-1):
            #     for jj in range(1, x_mask.shape[1]-1):
            #         if x_mask[ii][jj]!=0:
            #             num_pre_point=num_pre_point+1
            #             if x_mask[ii-1][jj-1]+x_mask[ii-1][jj]+x_mask[ii-1][jj+1]+ x_mask[ii][jj-1]+x_mask[ii][jj]+x_mask[ii][jj+1]+x_mask[ii+1][jj-1]+x_mask[ii+1][jj]+x_mask[ii+1][jj+1]<3.0:
            #                 num_not_boundary_point=num_not_boundary_point+1
            # ratio_tmp=num_not_boundary_point/num_pre_point
            # ratio_not_boundary_point=1.0/math.log10(1.0/(ratio_tmp+0.000001))
            # # loss公式
